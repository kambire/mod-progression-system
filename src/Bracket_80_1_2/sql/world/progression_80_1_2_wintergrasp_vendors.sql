...
-- Populate backup once (idempotent)
...
-- STRICT MODE (optional): wipe vendor first
DELETE nv
FROM npc_vendor nv
JOIN tmp_wg_vendor_entries v ON v.entry = nv.entry
WHERE @STRICT_MODE = 1;

-- Delete future-season items (tagged) beyond allowed season
DELETE nv
FROM npc_vendor nv
JOIN tmp_wg_vendor_entries v ON v.entry = nv.entry
...
-- Restore allowed items for this bracket
